...
The document looks clean and formatted. However, I think it look pretty bland to me. All of the raw symbol does not appear in the documents.
...
The .html file is saved in the ica folder. I can open but not edit it.
...
`#`this means headtitle

`-`
this means bullet point 

`\`
this uses to make the characters appears only on qmd not on the .html

`![](url)`
this means insert the link 
...
It appears to be only text but not the result of the code chunk itself after running 
...
When I put the `seq()` code in the chunk and run the green arrow it will show the result on the .qmd 
- Render. What appears in the html: R code, output, or both?

When I rendered it, the code is not formatted in text but rather in appear as a code chunk with the result of the code itself. 
...
```{r}
# c is celcius and f is fahrenheit
temperature_c <- 10
